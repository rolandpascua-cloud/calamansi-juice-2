...
- **`calamansi-juice-server.service` runs on port 13306, not config.json's default 13305** (`ExecStart=... calamansid --port 13306` in `data/calamansi-juice-server.service.in`). Found the same way as the `StateDirectory=` collision above: a real stock-Lemonade `lemond.service` on this fork's dev box defaults to 13305 too, and when both were enabled, whichever started second failed to bind and looped in `Restart=on-failure` while the other kept answering — silently serving stock Lemonade's routes (no `history`/`system/state`/`resources` endpoints) under what looked like a normal `200`, surfacing to the user only as 404s on this fork's own GUI panels. `README.md`'s "Running Calamansi Juice 2 (Linux, from source)" section documents 13306 as the systemd-managed port; a manually-run instance (no `--port` override) still defaults to 13305.
